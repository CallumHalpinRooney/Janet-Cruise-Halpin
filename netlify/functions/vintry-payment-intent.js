// Creates a Stripe PaymentIntent for a Vintry order.
//
...
exports.handler = async (event) => {
  ...
  if (!process.env.STRIPE_SECRET_KEY) {
    return { statusCode: 500, body: JSON.stringify({ error: 'Payments are not configured yet (missing STRIPE_SECRET_KEY).' }) };
  }

  try {
    const { items = [], delivery = 'collect', email = '' } = JSON.parse(event.body || '{}');
    ...
    for (const it of items) {
      ...
      if (unit == null || !(qty > 0) || qty > 99) {
        return { statusCode: 400, body: JSON.stringify({ error: `Item unavailable: ${it.id}` }) };
      }
      subtotal += unit * qty;
    }
    ...
    if (!res.ok) {
      return { statusCode: res.status, body: JSON.stringify({ error: data.error ? data.error.message : 'Payment error' }) };
    }
